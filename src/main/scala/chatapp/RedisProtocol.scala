...
trait RedisProtocol[F[_]] {
  ...
  def getSelectedUsers(userid: UserId, rest: List[UserId]): F[Option[List[User]]]
  def chatState: F[String]
}

object RedisProtocol {
  def make[F[_]: Monad](redis: RedisCommands[F, String, String]): F[RedisProtocol[F]] = {
    new RedisProtocol[F] {
      override def usernameExists(username: String): F[Boolean] = {
        redis.hVals("users").map { namelist =>
          namelist.exists(u => u.toLowerCase == username.toLowerCase)
        }
      }
